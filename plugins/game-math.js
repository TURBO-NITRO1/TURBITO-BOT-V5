...
`.trim()
  ...
  let id = m.chat
    if (id in conn.math) return conn.reply(m.chat, '📵️ Todavía hay dificultades sin respuesta de este chat 🤖', conn.math[id][0])
    let math = genMath(mode)
    conn.math[id] = [
        await conn.reply(m.chat, `᳆⃟✪ CUANTO ES 😎 ${math.str}*=\n\n_Tiempo:_ ${(math.time / 1000).toFixed(2)} segundos\n\n🎊 Recompensa : ${math.bonus} 🪙`, m),
        math, 4,
        setTimeout(() => {
            if (conn.math[id]) conn.reply(m.chat, `⏱️ Tú tiempo terminó!\nLa respuesta es : *${math.result}*`, conn.math[id][0])
      delete conn.math[id]
        }, math.time)
    ]
}
